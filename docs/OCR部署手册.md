...
实验室服务器是两张 RTX 5090，属于 NVIDIA Blackwell。PaddleOCR 官方 Blackwell 教程要求
驱动支持 CUDA 12.9 或以上，而当前服务器驱动 570.133.07 显示 CUDA 12.8。

因此执行 VL OCR 的正确顺序是：

1. 向服务器管理员确认是否允许升级 NVIDIA 驱动。
2. 确认服务器上没有依赖当前驱动的其他任务。
3. 由管理员升级到支持 CUDA 12.9 的驱动并重启、复核 `nvidia-smi`。
4. 选择本手册 6.2 节的 `.venv-ocr` 手动安装，或由管理员配置 Docker 和 NVIDIA
   Container Toolkit 后使用官方 `sm120` 镜像。
5. 先处理 3 页样本，再扩大到 10 份代表性资料。

不能在当前 CUDA 12.8 状态下直接照搬普通 CUDA 12.6 镜像，因为 RTX 5090 需要 Blackwell
专用路径。

`nvidia-smi` 显示的 CUDA 版本是驱动可支持的最高 CUDA 版本，并不表示已经安装
CUDA Toolkit；不要通过只在 `.venv-ocr` 里安装 Python 包来试图跳过驱动检查。
驱动升级属于整台共享服务器的变更，必须由管理员安排。
...
成功后检查 Markdown 的页数、表格、数字和来源，再考虑扩大样本。第一轮只用 GPU 0；
另开终端用 `nvidia-smi` 观察显存和进程。GPU 1 暂时不需要参与。
