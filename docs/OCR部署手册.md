...
对于不是纯白、但经人工查看确认只有页眉、页脚、页码或封底且没有业务内容的页面，可通过
`--page-exceptions` 传入 JSONL 例外台账。每条记录必须包含 `source`、当前原文件
`sha256`、`pages`、`reason`、`reviewer` 和 `reviewed_at`。缺字段、越界页、当前并未缺失的
页码或校验值不一致都会令严格验收失败，防止旧审核结论被套用到已替换文件。
